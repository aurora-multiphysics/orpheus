...
registerMooseObject("orpheusApp", Resistivity);
registerMooseObject("orpheusApp", ADResistivity);

template <bool is_ad>
InputParameters
ResistivityTempl<is_ad>::validParams() 
{
  ...
  params.addClassDescription("Calculates resistivity as a function of "
                             "temperature, using copper for parameter defaults.");
  return params;
}

template <bool is_ad>
ResistivityTempl<is_ad>::ResistivityTempl(const InputParameters & parameters)
  : Material(parameters),
    ...
    _resistivity(
        declareGenericProperty<Real, is_ad>(_base_name + "resistivity")),
    _dresistivity_dT(declareProperty<Real>(_base_name + "resistivity_dT"))
{
}

template <bool is_ad>
void
ResistivityTempl<is_ad>::computeQpProperties()
{
  ...
  _resistivity[_qp] = resistivity;
  _dresistivity_dT[_qp] = dresistivity_dT;
}

template class ResistivityTempl<false>;
template class ResistivityTempl<true>;
